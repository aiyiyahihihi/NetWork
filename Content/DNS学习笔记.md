...
> 域名系统 —— 互联网的电话簿，将人类可读的域名转换为机器可读的IP地址。
...
```
域名（www.example.com）  ──解析──▶  IP地址（93.184.216.34）
```
...
| 问题 | 说明 |
|:----:|------|
| 人类记忆 | 人类难以记忆大量IP地址（如：`142.250.80.46`），域名更直观 |
| IP可变性 | 服务器IP可能变更，域名提供稳定的访问入口 |
| 负载均衡 | 同一域名可解析到多个IP，实现流量分发 |
| 服务抽象 | 域名与底层基础设施解耦 |
...
- 标签不能以连字符开头或结尾
- **国际化域名（IDN）**：通过Punycode编码支持非ASCII字符（如：中文域名）
...
| 协议 | 端口 | 用途 |
|:----:|:----:|------|
| **UDP** | 53 | 标准查询（报文 < 512字节） |
| **TCP** | 53 | 区域传输（AXFR/IXFR）、大响应（EDNS0扩展后 > 512B） |
